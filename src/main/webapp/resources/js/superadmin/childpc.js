$(function () {
    var addUrl = "/superadmin/addchildpc";
    var deleteUrl = "/superadmin/removepc";

    /*$('#new')
        .click(
            function () {
                var tempHtml='<div class="row row-product-category temp">'
                +'<div class="col-33"><input class="category-input category" type="text" placeholder="子分类名"></div>'
    ...
    $('#submit').click(function () {
        ...
        tempArr.map(function (index,item) {
            ...
            tempObj.parent={
                productCategoryId:$('#new').attr('data-id')
            }
            if (tempObj.productCategoryName && tempObj.priority&&tempObj.parent) {
                pcList.push(tempObj);
            }
        });
        $.ajax({
